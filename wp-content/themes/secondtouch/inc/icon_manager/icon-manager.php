...
if(!class_exists('Crum_Icon_Manager'))
{
	class Crum_Icon_Manager
	{
		...
		function __construct()
		{
			...
			$this->paths['fonts'] 	= 'secondtouch_fonts';
...
';
		...
		function add_font()
		{
			...
			update_option('secondtouch_fonts', $fonts);
		}
		...
		static function load_iconfont_list()
		{
			...
			$extra_fonts = get_option('secondtouch_fonts');
		...
		function AIO_move_fonts()
		{
			...
			if ( !file_exists($this->vc_fonts.'/moon/moon.ttf') ) {
				if ( ! is_dir( $this->vc_fonts.'/moon' ) ) {
					wp_mkdir_p( $this->vc_fonts.'/moon' );
				}
				@chmod( $this->vc_fonts, 0777 );
				foreach ( glob( $this->vc_fonts_dir . '/moon/*' ) as $file ) {
					$new_file = basename( $file );
					@copy( $file, $this->vc_fonts . '/moon/' . $new_file );
				}
				$fonts['moon'] = array(
					'include' => trailingslashit( $this->paths['fonts'] ) . 'moon',
					'folder'  => trailingslashit( $this->paths['fonts'] ) . 'moon',
					'style'   => 'moon' . '/' . 'style' . '.css',
					'config'  => $this->paths['config']
				);

			}
			if ( !file_exists($this->vc_fonts.'/linecon/fontello.ttf') ) {
				if ( ! is_dir( $this->vc_fonts.'/linecon' ) ) {
					wp_mkdir_p( $this->vc_fonts.'/linecon' );
				}
				@chmod( $this->vc_fonts, 0777 );
				foreach ( glob( $this->vc_fonts_dir . '/linecon/*' ) as $file ) {
					$new_file = basename( $file );
					@copy( $file, $this->vc_fonts . '/linecon/' . $new_file );
				}
				$fonts['linecon'] = array(
					'include' => trailingslashit( $this->paths['fonts'] ) . 'linecon',
					'folder'  => trailingslashit( $this->paths['fonts'] ) . 'linecon',
					'style'   => 'linecon' . '/' . 'style' . '.css',
					'config'  => $this->paths['config']
				);

			}
			$defaults      = get_option( 'secondtouch_fonts' );
			if ( ! $defaults ) {
				update_option( 'secondtouch_fonts', $fonts);
			}
