...
function onCreate (event) // called only once - bind events here
{
    ...
    j$("#btn_add_acc").attr("title", stringres.get("addeditct_hint_adddetails"));
...
var atemplate = '' +
    '<div class="acc_item" id="acc_item_[NR]">' +
        '<div class="acc_control">' +
            '<input type="checkbox" class="acc_checkbox" id="acc_checkbox_[NR]" name="acc_checkbox_[NR]" [ENABLE_CHECKED]>' +
            '<input type="radio" class="acc_radio" id="acc_radio_[NR]" name="acc_active_selection" [MAIN_CHECKED]>' +
        '</div>' +
        '<div class="acc_username" id="acc_username_[NR]">[USERNAME]</div>' +
        '<div class="acc_remove"><button id="btn_remove_account_[NR]" class="btn_remove_account noshadow ui-btn-inline ui-btn ui-btn-corner-all ui-btn-b ui-icon-minus ui-btn-icon-notext">Remove</button></div>' +
...
'<div id="addaccountpopup" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' + popupWidth + 'px;">' +
    ...
    '<div role="main" class="ui-content adialog_content">' +
        ...
        '<input type="text" id="addacc_serveraddress" name="addacc_serveraddress" data-theme="a"/>' +
        
        '<br /><label for="addacc_username">' + stringres.get('addacc_user') + ':</label>'+
        '<input type="text" id="addacc_username" name="addacc_username" data-theme="a"/>' +
        
        '<br /><label for="addacc_password">' + stringres.get('addacc_password') + ':</label>'+
        '<input type="text" id="addacc_password" name="addacc_password" data-theme="a"/>' +
        
        '<br /><label for="addacc_ival">' + stringres.get('addacc_ival') + ':</label>'+
        '<input type="text" id="addacc_ival" name="addacc_ival" value="3600" data-theme="a"/>' +
    '</div>' +
...
*/
        ...
        j$('#adialog_positive').on('click', function (event)
        {
            ...
            j$( '#addaccountpopup' ).on( 'popupafterclose', function( event )
            {
                ...
                var acc = new common.ClassSipAccounts();
                
                acc.serveraddress = srv;
                acc.username = usr;
                acc.password = pwd;
                acc.ival = common.StrToInt(ivalstr);
                acc.enabled = true;
