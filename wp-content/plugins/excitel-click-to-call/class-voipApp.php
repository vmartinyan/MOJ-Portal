...
class VoipApp {
    ...
    public static function activate( ) {
        ...
        $excitel_prefs_table = self::get_table_handle();
        $charset_collate = '';
        if (version_compare(mysql_get_server_info(), '4.1.0', '>=')) {
            $charset_collate = "DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci";
        }

        if ($wpdb->get_var("SHOW TABLES LIKE '$excitel_prefs_table'") != $excitel_prefs_table) {
            $sql = "CREATE TABLE `" . $excitel_prefs_table . "` (
    ...
    public static function deactivate( ) {
        ...
        $sql = "DROP TABLE $excitel_prefs_table";
        $wpdb->query($sql);
    }
