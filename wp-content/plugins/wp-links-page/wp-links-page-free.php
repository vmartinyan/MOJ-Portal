...
function wp_links_page_free_event_hook() {
	...
	$screenshot = file_get_contents('https://www.googleapis.com/pagespeedonline/v1/runPagespeed?url='.$link->url.'&screenshot=true');
    ...
    $file = WPLPF_UPLOAD_DIR . $filename;
	...
	if ($success != false) {
		$cur_img = basename($link->img);
		$cur_image = WPLPF_UPLOAD_DIR . $cur_img;
		if(file_exists($cur_image)) unlink($cur_image);
	}
...
function wp_links_page_free_options() {
	...
	$i = 1;
	foreach ($links as $link) {
		$display = stripslashes( $link->display );
		$desc = stripslashes( $link->description );
		$table_rows .= '<tr id="'.$link->id.'" ><td id="'.$link->id.'" class="index">'.$i.'</td><td class="screenshot" ><img src="'.$link->img.'" /></td><td class="url" >'.$display.'</td><td class="description" >'.$desc.'</td><td class="edit"><button id="'.$link->id.'" class="update button button-primary button-large" style="display: none;">Update</button><button id="'.$link->id.'" class="edit button button-primary button-large">Edit</button></td><td id="'.$link->img.'" class="delete" ><button id="'.$link->id.'" class="delete button button-primary button-large">Delete</button></td></tr>';
		$i++;
	}
	echo '<h1>Edit WP Links Page Free Setup</h1>
			...
			<label for="display" >Link Display</label>
			<p class="description">This field defaults to the link domain.</p>
			<input id="display-input" type="text" name="display" maxlength="255" />
...
function wp_links_free_subpage_options() {
	...
	if(isset($_GET['settings-updated']) && $_GET['settings-updated'])
   {
	...
	echo '<tr><th scope="row"><label class="label" for="wplpf_width" >Grid Width</label></th>
		...
		echo ' >5</label><br/>
		<p class="description" >How many columns should the grid have?<br />The number of columns is currently set to '.$width.'.</p></td></tr>';
		echo '<tr><th scope="row"><label class="label" for="wplpf_cron" >Flush Cron Jobs</label></th>
		<td>
		<div id="wplpf-flush-cron" class="button button-primary button-large">Flush Cron Jobs</div>
		<br/>
		<img class="progress" style="margin-top: 10px; display: none;" src="'.plugins_url( "images/ajax-loader.gif", __FILE__ ).'" >
		<p class="message" style="display:none">Cron jobs have been flushed.</p>
		<p class="description" >If you are having trouble with updating your screenshots properly, you may need to flush your cron events.<br/>';
...
function wp_links_free_help_page() {
	...
	<a href="http://www.wplinkspage.com/purchase/" target="_blank" class="wplpf-help-p">Click here to purchase the pro version and take advantage of the fine tuned controls available such as shortcodes, categories, and pagination.</a><br/><br/>
	<h2>Documentation</h2>
...
<p>A new widget is also provided. This widget will display a one column grid of the links with descriptions.</p>
...
<div class="fusion-one-half fusion-layout-column fusion-spacing-yes" style="margin-top:0px;margin-bottom:20px;"><div class="fusion-column-wrapper"><h3><strong>WP Links Page Pro</strong></h3>
<p >Dynamic Links with Live Screenshots for your WordPress Website</p>

<p>A WordPress plugin that allows you to create a dynamic link gallery with screenshots of each link. It will automatically create screenshots of each link and save you from creating a hardcoded links page or finding images for each site. It allows sites to create a links page with several different views. The screenshots for the links can be updated on a set schedule or with the click of a button. Links can be added, edited, and reordered on the setup page. The <a href="http://www.wplinkspage.com/" target="_blank">Pro Version of WP Links Page</a> has a lot of extra features:</p>
<ul style="list-style:inside none disc;margin-left:15px;">
...
<li>Screenshots can be updated on schedule or by command.</li>
<li>HTML Can be added in the descriptions</li>
<li>Display your links by categories.</li>
<li>Pagination can be used for large amounts of links.</li>
<li>Replace any screenshots with your own custom image.</li>
<li>Shortcodes with fine tuned controls to display your links in many styles.</li></ul>
<p>Shortcode options include: </p>
	<p><strong>&#91;wp_links_page]</strong></p>
	<ul>
	<li>Displays all links with the global settings.</li>
	</ul>
	...
	<p><strong>&#91;wp_links_page sort=&quot;random&quot;]</strong></p>
	<ul>
	<li>Displays Links in a Random order on every page load. We do not recommend using this with pagination.</li>
	<li><strong>Allowed Options: </strong>random</li>
	</ul>
</div></div><div class="fusion-one-half fusion-layout-column fusion-column-last fusion-spacing-yes" style="margin-top:0px;margin-bottom:20px;"><div class="fusion-column-wrapper"><p><img class="alignnone wp-image-184 size-full" src="'.plugins_url( "images/Links-page-3-Col.jpg", __FILE__ ).'" alt="Shortcode Example" /></p>
</div></div><div class="fusion-clearfix"></div><div class="fusion-sep-clear"></div><div class="fusion-separator fusion-full-width-sep sep-single" style="border-color:#e0dede;border-top-width:1px;margin-left: auto;margin-right: auto;margin-top:px;margin-bottom:30px;"></div>
<a href="http://www.wplinkspage.com/" target="_blank">For more information please visit us at wplinkspage.com</a></div>';
}
...
function wp_links_page_free_links() {
	...
	$content = '';
	$content .= '<style>.wplpf-wrapper {overflow: auto; zoom: 1; clear: both;}
		.wplpf-link {margin-bottom: 10px;}
		...
		@media screen and (min-width: 55em) {.wplpf-grid{width: '.$width.'%;}}</style><div class="wplpf-wrapper">';
	global $wpdb;
	$table = $wpdb->prefix.'wp_links_page_free_table';
	$links = $wpdb->get_results("SELECT * FROM $table ORDER BY weight ASC");
	$i = 0;
	foreach ($links as $link) {
		...
		$display = stripslashes( $link->display );
		$desc = stripslashes( $link->description );
		$content .= '<a class="wplpf-link wplpf-'.$grid.$clear.'" href="'.$link->url.'" target="_blank"><div class="wplpf-link-grid"><img src="'.$link->img.'" ><p class="url">'.$display.'</p>';
		if ($grid == 'list') {
		$content .= '<p class="wplpf-description">'.$desc.'</p><hr style="clear:both;"></div></a>';
		} else { $content .= '</div></a>';}
	}
	$content .= '</div>';
	return $content;
...
class WP_Links_Free_Widget extends WP_Widget {
	...
	$content = '<style>.wplpf-wrapper {overflow: auto; zoom: 1; clear: both;}
		.wplpf-link-grid-widget { text-align: center; }
		...
		.wplpf-description-widget {font-style: italic;}</style><div class="wplpf-wrapper">';
	global $wpdb;
	$table = $wpdb->prefix.'wp_links_page_free_table';
	$links = $wpdb->get_results("SELECT * FROM $table ORDER BY weight ASC");
	$i = 0;
		...
		$display = stripslashes( $link->display );
		$desc = stripslashes( $link->description );
		$content .= '<a class="wplpf-link-widget wplpf-widget" href="'.$link->url.'" target="_blank"><div class="wplpf-link-grid-widget"><img src="'.$link->img.'" ><p class="url">'.$display.'</p><p class="wplpf-description-widget">'.$desc.'</p></div></a>';
		}
		$content .= '</div>';
...
function wplpf_ajax_callback() {
    ...
    $img_url = WPLPF_UPLOAD_URL.$file.$time.".jpg";
	
    $file = WPLPF_UPLOAD_DIR . $file . $time . '.jpg';
...
function wplpf_ajax_delete_callback() {
	...
	if (isset($_REQUEST['id'])) {
		...
		$del = $wpdb->get_results("SELECT * FROM $table WHERE id = '$id' ORDER BY weight ASC");
		$file = $del->img;
		$cur_img = basename($file);
		$cur_image = WPLP_UPLOAD_DIR . $cur_img;
		if(file_exists($cur_image)) unlink($cur_image);
...
function wplpf_ajax_update_screenshots_callback() {
	...
	if (isset($data_whole->screenshot->data)) {
		...
		error_log('data: '.$data_whole);
		error_log('screenshot: '.$screenshot); die();}
...
add_action( 'wp_ajax_wplpf_ajax_flush_cron', 'wplpf_ajax_flush_cron');
function wplp_ajax_flush_cron() {
		$sr = get_option('wplpf_screenshot_refresh');
		$exists = wp_get_schedule( 'wp_links_page_free_event' );
		if ($exists == false) {
			wp_schedule_event(time(), $sr, 'wp_links_page_free_event');
