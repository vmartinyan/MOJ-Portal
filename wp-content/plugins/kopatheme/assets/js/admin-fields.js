...
var Kopa_UI_Image = {
	init: function () {
		jQuery('.kopa-field-image').on('click', '.item-add', function (e) {
			...
			image_field_frame[id] = wp.media.frames.downloadable_file = wp.media({
			...
			image_field_frame[id].on('select', function () {
				var attachment = image_field_frame[id].state().get('selection').first().toJSON();
